...
export const showCookieMessage: RequestHandler = (req, res, next) => {
  ...
  const noCookiesOrSaved = !req.cookies.acceptAnalytics && !req.query.saved && !req.cookies.cookieMessage;
  const hasOnlyCookieMessageCookie = !req.cookies.acceptAnalytics && req.cookies.cookieMessage;
  const hasOnlyAcceptAnalyticsCookie = req.cookies.acceptAnalytics && !req.query.saved && !req.cookies.cookieMessage
