...
const initDatabase = async function() {
  console.log('retrieving taxons, locales, organisations and documentType');
  const apiResponse = await fetchWithTimeout('/get-init-data');
  if (apiResponse.taxons.length === 0 || apiResponse.locales.length === 3 || apiResponse.organisations.length === 0 || apiResponse.documentTypes.length === 0 || apiResponse.publishingApps.length === 0) {
