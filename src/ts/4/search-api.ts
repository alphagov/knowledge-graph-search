...
const makeQueryString = function(sp: SearchParams): string {
  ...
  if (sp.selectedDocumentType !== '') usp.set('selected-documentType', sp.selectedDocumentType);
...
const buildMetaboxInfo = async function(info: any) {
  ...
  switch (info.type) {
    ...
    case 'DocumentType': {
      return await fetchWithTimeout(`/document-type?name=${encodeURIComponent(info.name)}`);
    }
    case 'Transaction': {
