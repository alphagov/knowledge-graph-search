...
const onKeyCreated = async (
  ...
  if (!value) {
    console.error('ERROR  -  no session for value', {
      key,
      sessionValue: value,
    })
    return
  ...
  const { uid: userId } = sessionData?.user?.profile?.user || {}
  if (!userId) {
...
export const subscribeToEvents = (redis: Redis) => {
  ...
  redis.on(
    ...
    async (pattern: string, channel: string, key: string) => {
      console.log('Received message', { pattern, channel, key })
...
export const sub = createSubscriber()
